		...
		crypt_pair_t* const target_crypt, crypt_pair_t* const target_decrypt)
{
	...
	*/
	...
	const char* const section_names = (char*)map->addr + shdr[ehdr->e_shstrndx].sh_offset;

	uqword amount = 0;
	while (target_crypt[amount].start != 0)
		amount++;
...
//			".text"
		...
		{
			if (ft_strcmp(tofind[y], section_names + shdr[i].sh_name) == 0)
			{
